...
analyse_textfile_per_sentence <- function(filename) {
  ...
  for (i in 1:length(filesplit)){
    ...
    ratio <- nchar(sentence)/nchar(file)
...
#Make a 'drift column'
scaledhrdata1$driftdown <- c(-6999:7000)
scaledhrdata1$driftup <- c(7000:-6999)
...
#Scaling sentidascores
scale(scaledhrdata1$sen)
...
newtest <- scaledhrdata1 %>% 
  pivot_longer(
    cols = starts_with(scaledhrdata1$`001`), 
    names_to = "week", 
    values_to = "rank",
    values_drop_na = TRUE
  )
